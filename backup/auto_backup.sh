...
# Обновляем конфигурацию бэкенда, добавляя переменную backup_disks
log "Обновление конфигурации бэкенда, добавление переменной backup_disks..."
# Проверяем наличие Node.js
if command -v node &> /dev/null; then
    # Делаем скрипт исполняемым, если он еще не исполняемый
    chmod +x "$SCRIPT_DIR/update_backup_config.js"
    
    # Запускаем скрипт обновления конфигурации
    node "$SCRIPT_DIR/update_backup_config.js"
    if [ $? -eq 0 ]; then
        log "Конфигурация бэкенда успешно обновлена"
    else
        log "ПРЕДУПРЕЖДЕНИЕ: Не удалось обновить конфигурацию бэкенда"
    fi
else
    log "ПРЕДУПРЕЖДЕНИЕ: Node.js не найден, невозможно обновить конфигурацию бэкенда"
fi
